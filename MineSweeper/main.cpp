//
// Created by Vojtěch Kratina on 07.12.2023.
//
#include <iostream>
#include <fstream>
#include <sstream>
...
int main() {
    ...
    std::cout << "Welcome to mine sweeper! \nThis is your board setup" << std::endl;
    showHelp();
    creatingBoard();
    printMines();
    ...
    while (!GameOver) {
        ...
        while (true) {
            ...
            if (BOARD_SIZE.first - 1 >= x && BOARD_SIZE.second - 1 >= y && x >= 0 && y >= 0) {
                break;
            } else {
                std::cout << "Please choose x and y in this range 0 <= x <= " << BOARD_SIZE.first - 1
                          << " and 0 <= y <= " << BOARD_SIZE.second - 1 << std::endl;
                if (visited.empty()) {
                    revealedCells = 0;
                }
            }
        ...
        if (MINES[y][x] == "X " && !MARK) {
            ...
            revealedCells = revealCell(x, y, revealedCells);
            if (revealedCells >= ((BOARD_SIZE.second * BOARD_SIZE.first) - NUM_MINES)) {
